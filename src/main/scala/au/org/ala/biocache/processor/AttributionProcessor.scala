    ...
    if (raw.attribution.dataResourceUid != null) {
      ...
      if (!dataResource.isEmpty) {
        ...
        if (dataResource.get.hasMappedCollections && (raw.occurrence.collectionCode != null || processed.occurrence.collectionCode != null)) {
          ...
          val collCode = if (raw.occurrence.collectionCode != null) {
            raw.occurrence.collectionCode
          } else {
            processed.occurrence.collectionCode
          }
          //use the collection code as the institution code when one does not exist
          val instCode = if (raw.occurrence.institutionCode != null) {
            raw.occurrence.institutionCode
          } else if (processed.occurrence.institutionCode != null) {
            processed.occurrence.institutionCode
          } else {
            collCode
          }
          ...
          if (!attribution.isEmpty) {
            ...
            assertions ++= Array(QualityAssertion(UNRECOGNISED_COLLECTIONCODE, PASSED))
            assertions ++= Array(QualityAssertion(UNRECOGNISED_INSTITUTIONCODE, PASSED))
        ...
        if (raw.attribution.license == null) { //if no record-level licence then use dataset-level licence
          processed.attribution.license = dataResource.get.license
        } else {
          val term = License.matchTerm(raw.attribution.license)
          if (!term.isEmpty) {
            processed.attribution.license = term.get.canonical
          } else {
            processed.attribution.license = "other"
          }
        }
