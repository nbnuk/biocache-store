  ...
  def main(args: Array[String]) {
    ...
    if (parser.parse(args)) {
      ...
      if (abortIfNotRowKeyFile && (rowKeyFile == "" || !(new File(rowKeyFile).exists()))) {
        ...
        val layers = {
          if(singleLayerName != "") Array(singleLayerName)
          else Array[String]()
        }
    ...
    val pool: Array[StringConsumer] = Array.fill(threads) {
      ...
      val p = new StringConsumer(queue, ids.incrementAndGet(), sentinel, { guid =>
        if(!guid.trim().isEmpty()) {
          ...
          if(!result.isEmpty){
            val map = result.get
            val lat = map.get(dlat).get //NBN
            val lon = map.get(dlon).get //NBN
...
class PointsReader(filePath: String) {
  ...
  def loadPoints(max: Int): Array[Array[Double]] = {
    ...
    while (current != null && count < max) {
      try {
        if(current.length == 2 && current(0) != "null" && current(0) != "null"){
...
//NBN case sensitive fix
  ...
  def getDistinctCoordinatesForResourceThreaded(numThreads: Int, locFilePath: String, dataResourceUid: String = "") {
    ...
    val lce = new LocColumnExporter(numThreads, dataResourceUid, handleRecordMap)
    lce.run
    try {
      val fw = new FileWriter(locFilePath)
...
class LoadSamplingConsumer(batches: LinkedBlockingQueue[String]) extends Thread {
  ...
  val logger = LoggerFactory.getLogger("LoadSamplingConsumer")
