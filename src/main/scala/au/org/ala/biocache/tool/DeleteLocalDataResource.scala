...
class DeleteLocalDataResource {
  ...
  def delete(entityName: String, threads: Int, drs: Seq[String], startTokenRangeIdx: Int): Unit = {
    ...
    val recordsRead = Config.persistenceManager.pageOverLocal("occ", (rowkey, map, batchID) => {
      val drUid = map.getOrElse((if (Config.caseSensitiveCassandra) "dataResourceUid" else "dataresourceuid"), "")//NBN
      ...
      true
    }, threads, Array("rowkey", (if (Config.caseSensitiveCassandra) "dataResourceUid" else "dataresourceuid")))//NBN
